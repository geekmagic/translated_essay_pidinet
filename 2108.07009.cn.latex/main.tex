...
正如之前的一些工作~\cite{wibisono2020fined, poma2020dense, wibisono2020traditional}所尝试的那样，我们认为通过构建一个模型尺寸小、运行效率高的架构，一次性解决第~\ref{sec:intro}节中提到的低效率问题是必要和可行的，并且可以使用有限的数据集从头开始训练，以实现有效的边缘检测。我们用以下部分构建我们的架构（Fig. ~\ref{fig:arch}）。
...
\noindent \textbf{高效的主干网络。} \quad 主干网络的构建原则是在具有高运行效率的情况下使结构变得苗条。因此我们没有考虑为许多其他任务提出的复杂的多分支轻量级结构~\cite{gao2020100k, mehta2019espnetv2, yu2018bisenet}，因为它们可能不适合并行实现~\cite{ma2018shufflenetv2}，从而导致对边缘检测任务的效率不令人满意。受\cite{he2016residual}和\cite{howard2017mobilenets}的启发，我们使用可分离的深度卷积结构以及快速推理和简单训练的快捷方式。整个主干有4个阶段，其中最大池化层用于下采样。每个阶段有4个残差块（第一阶段有一个初始卷积层和3个残差块除外）。每个块中的残差路径依次包括深度卷积层、ReLU层和点卷积层。每个阶段中的通道数量相当少，以避免模型尺寸过大（阶段1、2、3和4分别为$ C $、$ 2 \times C $、$ 4 \times C $和$ 4 \times C $通道）。
...
\noindent \textbf{高效的侧面结构。} \quad 为了学习丰富的分层边缘表示，我们还使用了~\cite{xie2017holistically}中的边结构分别从每个阶段生成一个边缘图，使用实际真实图计算边损失，以提供深度监督~\cite{xie2017holistically}。为了细化特征图，从每个阶段的末尾开始，我们首先构建了一个基于紧凑膨胀卷积的模块（CDCM）来丰富多尺度边缘信息，它以$ n \times C $通道为输入，并在输出中产生$ M $（$ M < C $）通道，以减轻计算开销，然后是紧凑的空间注意模块（CSAM）以消除背景噪声。之后，$ 1 \times 1 $卷积层进一步将特征量减少到单个通道图，然后将其插值到原始大小，然后使用Sigmoid函数创建边缘图。用于测试的最终边缘图是通过将4个单通道特征图与串联、卷积层和Sigmoid函数融合而创建的。
...
详细的结构信息可以在Fig. ~\ref{fig:arch}中看到，注意为了简单起见，我们没有使用任何归一化层，因为训练图像的分辨率不统一。获得的架构是我们的基线。通过用PDC替换残差块中$ 3 \times 3 $深度卷积层中的普通卷积，我们得到了提议的PiDiNet。
...
\noindent \textbf{损失函数。} \quad 我们采用~\cite{liu2019richer}中提出的注释器鲁棒损失函数用于每个生成的边缘图（包括最终的边缘图）。对于第$ j $个边缘图中值为$ p_i^j $的第$ i $个像素，损失计算如下：
...
其中$ y_i $是真实值边缘概率，$ \eta $是一个预定义的阈值，这意味着如果一个像素被少于$ \eta $的注释器标记为正以避免混淆，则该像素将被丢弃，且不被视为样本，$ \beta $是负像素样本的百分比，$ \alpha = \lambda \cdot (1 - \beta) $。总之，总损失是$ L = \sum_{i,j} l_i^j $。
...
\noindent \textbf{实验数据集。} \quad 我们在三个广泛使用的数据集上评估了提议的PiDiNet，即BSDS500~\cite{arbelaez2010bsds}、NYUD~\cite{shi2000nyud}和Multicue~\cite{mely2016multicue}。关于三个数据集的数据增强和配置的实验设置如下~\cite{xie2017holistically, liu2019richer, he2019bidirectional}，详细信息如下。BSDS500分别由训练集、验证集和测试集中的200、100和200张图像组成。每个图像有4到9个注释器。数据集中的训练图像通过翻转（2$ \times $）、缩放（3$ \times $）和旋转（16$ \times $）进行增强，从而产生比未经整理的版本大96$ \times $的训练集。与之前的作品~\cite{xie2017holistically, liu2019richer, he2019bidirectional}一样，PASCAL VOC上下文数据集~\cite{mottaghi2014voc}具有10K标记图像（并通过翻转增加到20K），也可以选择在训练中考虑。NYUD有1449对对齐的RGB和深度图像，这些图像被密集标记。分别有381、414和654张图像用于训练、验证和测试。我们将训练和验证集结合起来，并通过翻转（2$ \times $）、缩放（3$ \times $）和旋转（4$ \times $）来增强它们以生成训练数据。Multicue由100个具有挑战性的自然场景组成，每个场景都包含由双目立体相机捕获的左视图和右视图颜色序列。我们的实验使用了每个场景的左视图序列的最后一帧，该帧用边缘和边界标记。我们将它们随机分成80张和20张图像，分别用于训练和评估。该过程独立地重复两次以上。然后从三次运行中记录指标。我们还使用翻转（2$ \times $）、缩放（3$ \times $）和旋转（$16 \times $）来增强每个训练图像，然后将它们随机裁剪为500$ \times $500的图像。
...
\noindent \textbf{性能指标。} \quad 在评估过程中，将为所有数据集记录最佳数据集尺度（ODS）和最佳图像尺度（OIS）下的\emph{F-measure}。由于效率是本文的主要关注点之一，如果未指定，所有模型都是基于单尺度图像的评估进行比较的。
